...
| Plugins in `enabledPlugins` | Turn off plugins they don't use. `/plugin` → **Installed** lists them under **Not used recently**; `claude plugin details <name>` shows each one's per-session token cost | Each enabled plugin adds skills, and sometimes tools, to every session. |
...
- **Keep the cache warm.** Claude Code reuses the unchanged start of the conversation cheaply for about an hour on a subscription (five minutes on an API key). Switching models mid-session, turning MCP servers (or plugins that include them) on or off, and `/compact` start the cache over; adding a skills-only plugin with `/reload-plugins` does not. Pick the model and tools at the start and keep them.
